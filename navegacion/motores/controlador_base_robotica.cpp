...
int main(int argc, char** argv){
  ros::init(argc, argv, "controlador_base_robotica");
  ...
  double rate = 10.0;
  double linear_scale_positive = 0.5;
  double linear_scale_negative = 0.5;
  double angular_scale_positive = 0.5;
  double angular_scale_negative = 0.5;
  ...
  while(n.ok()){
    ...
    dt = rpm_dt;
    dxy_ave = (rpm_act1+rpm_act2)*dt*diametro_ruedas*pi/(60*2);
    dth_odom = (rpm_act2-rpm_act1)*dt*diametro_ruedas*pi/(60*distancia_ruedas);
    ...
    dx = cos(dth_odom) * dxy_ave;   
    dy = -sin(dth_odom) * dxy_ave;  

    x_pos += (cos(theta) * dx - sin(theta) * dy);
    y_pos += (sin(theta) * dx + cos(theta) * dy);
    theta += dth_odom;   

    if(theta >= dos_pi) theta -= dos_pi;
    if(theta <= -dos_pi) theta += dos_pi;
    ...
    vth = (dt == 0)? 0 : dth_odom/dt; 
    odom_msg.child_frame_id = base_link;
    odom_msg.twist.twist.linear.x = vx;
    odom_msg.twist.twist.linear.y = 0.0;  
    odom_msg.twist.twist.angular.z = dth_odom; 
